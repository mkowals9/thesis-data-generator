...
for single_case in data:
    ...
    for wavelength in wavelengths:
        ...
        for index in range(1, M):
            ...
            if index != 1:
                R_i = matrix_f_i * all_R_matrices_per_wavelength[index-2] #bo indeksy tablicy mamy od 0,1 a index leci od 1
        ...
        for matrix in all_R_matrices_per_wavelength[1:]:
            result = np.dot(multiplied_R_matrices, matrix)
        final_output_R = np.array(multiplied_R_matrices) * R_0
        final_output_S = np.array(all_S_matrices_per_wavelength) * S_0
